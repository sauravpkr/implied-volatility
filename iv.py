...
#response = requests.get(url_indices,headers=api_headers)
#json_data=json.loads(response.text)

#filtered_data= json_data['filtered']  # filtered_data is of type Dict
...
def pull_iv(securities):
    ...
    json_data=json.loads(response.text)
    filtered_data= json_data['filtered']['data']  # filtered_data is of type Dict
        ...
        for second_level,x in first_level.items():
            if(second_level == 'PE'):
                ...
                for y ,y_item in x.items():
                    if(y == 'underlying'):
                        datalist.insert(1,y_item)  
                    elif(y == 'identifier'):
                        datalist.insert(0,y_item)      
                    elif(y == 'expiryDate'):
                        datalist.insert(2,y_item)
                    elif(y == 'strikePrice'):
                        datalist.insert(3,y_item)
                    elif(y == 'impliedVolatility'):
                        datalist.insert(4,y_item)
                    elif(y == 'openInterest'):
                        datalist.insert(5,y_item)
